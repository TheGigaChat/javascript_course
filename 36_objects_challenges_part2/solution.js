...
const alchemyLab = {
  ...
  craftPotion(potionName, inventory) {
    ...
    if (potionName === "speedPotion") {
      if (inventory["water"] >= 1 && inventory["air"] >= 2) {

        inventory["water"]--
        inventory["air"] -= 2
  
        const speedPotion = {speed: 50}
        return speedPotion
      } else {
        console.log("You don't have enough resourses.")
      }
    }

    // 2 fire, 1 earth => powerPotion{damage: 5}
    if (potionName === "powerPotion") {
      if (inventory["fire"] >= 2 && inventory["earth"] >= 1) {

        inventory["fire"] -= 2
        inventory["earth"]--

        const powerPotion = {damage: 5}
        return powerPotion
      } else {
        console.log("You don't have enough resourses.")
      }
    ...
    if (potionName === "healthPotion") {
      if (inventory["water"] >= 2 && inventory["earth"] >= 1) {
  
        inventory["water"] -= 2
        inventory["earth"]--
  
        const healthPotion = {hp: 30}
        return healthPotion
      } else {
        console.log("You don't have enough resourses.")
      }
